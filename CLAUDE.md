...
- Keep CI lean, but check what the gate COVERS before you check what it costs. A cheap required check that runs a fraction of the suite reports green and protects nothing — that is worse than a slow one, because nobody looks. The pull-request path is one job running everything that needs no infrastructure; database and browser suites go post-merge and nightly. Never `paths-ignore` a required check (it then never reports, and blocks the PR forever). Templates: `.claude/templates/ci.yml` + `nightly.yml` + `vitest.route.ts`. Story and rules: `docs/ci-leanness.md`. Audit an existing repo with `/ci-audit` — templates only reach new ones.
